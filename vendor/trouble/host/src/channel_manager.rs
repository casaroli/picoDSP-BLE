...
impl<'d, P: PacketPool> ChannelManager<'d, P> {
                ...
                L2capSignalCode::CONN_PARAM_UPDATE_REQ => {
                    ...
                    if !params.is_valid() {
                        ...
                        let req = ConnectionParamsRequest::new(
                            params,
                            conn,
                            // Echo this request's signaling identifier in the response.
                            header.identifier,
    ...
    pub(crate) async fn send_conn_param_update_res<T: Controller>(
        ...
        // A signaling response must echo the originating request's identifier (Core spec
        // Vol 3 Part A §4.1), not a freshly-allocated one — otherwise the peer can't match
        // it to its outstanding request.
        let mut tx = [0; 16];
        host.l2cap_signal(handle, identifier, param, &mut tx[..]).await
    }
