...
pub struct ConnectionParamsRequest {
    ...
    handle: ConnHandle,
    responded: bool,
    /// L2CAP signaling identifier of the originating request. A signaling response must
    /// echo the request's identifier (Core spec Vol 3 Part A §4.1); replying with a fresh
    /// identifier leaves the peer's RTX timer unsatisfied, so it tears the link down after
    /// ~30s with "Unacceptable Connection Parameters". Unused for LL-originated requests
    /// (those reply via HCI, not L2CAP signaling).
    identifier: u8,
    #[cfg(feature = "connection-params-update")]
    l2cap: bool,
}
        ...
        C: crate::Controller,
    {
        ...
        match stack.host.async_command(into_le_conn_update(self.handle, params)).await {
            Ok(()) => {
                ...
                stack.host.send_conn_param_update_res(self.handle, self.identifier, &param).await
            }
